...
export default function AlertForm() {
	const [state, handleSubmit] = useForm("xnnlalqy");
		...
		const observer = new IntersectionObserver(
			(entries) => {
				entries.forEach((entry) => {
					if (entry.isIntersecting && emailInputRef.current && !state.succeeded) {
			...
			<form onSubmit={handleSubmit} className="flex flex-col md:flex-row md:items-end md:justify-center gap-4">
				<div className="flex-1 lg:max-w-sm">
					...
					<ValidationError
						...
						field="email"
						errors={state.errors}
						className="text-lg font-light text-red-600 mt-2 text-left"
					/>
				</div>
				<div className="flex justify-center md:justify-start">
					...
					>
						{state.submitting ? (
							<b className="tracking-[-0.2em] font-lexend-zetta uppercase">ADDING YOU TO THE LIST...</b>
						) : (
